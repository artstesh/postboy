...
6. **Release announcement.** Finishing work on a version always includes drafting the announcement post for the library's Telegram channel: a short user-facing summary of what changed (version number, fixes, migration notes if any). Hand the draft to the user — nothing is ever posted automatically.
