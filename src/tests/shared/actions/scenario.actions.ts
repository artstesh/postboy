...
export class ScenarioActions {
  constructor(
    ...
    private message?: PostboyMessage,
  ) {}
  ...
  fireCallback(message?: PostboyCallbackMessage<any>, action?: (e: any) => void): void {
    ...
    this.world
      .getPostboy()
      .fireCallback(message ?? (this.message as PostboyCallbackMessage<any>))
      .subscribe(action ?? (() => {}));
  }

  subscribe(type: MessageType<any>, handler: (...args: any[]) => void = () => {}): Subscription {
    return SubscriptionBuilder.forType(this.world, type).withHandler(handler).subscribe();
  }

  once(type: MessageType<any>, handler: (...args: any[]) => void = () => {}): Subscription {
    return SubscriptionBuilder.forType(this.world, type).asOnce().withHandler(handler).subscribe();
  }
}
